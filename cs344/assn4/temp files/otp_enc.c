...
int main(int argc, char *argv[])
{
    ...
    n = write(sockfd, &size, sizeof(size));       //write out how large the buffer is
    if (n < 0)
        error("ERROR writing sizeof to socket");
    ...
    if(n < 0)
        error("ERROR writing sizeof to socket post encrypt");   //we're sending size again
