...
//This function accepts the text and key strings and returns a decrypted version
...
char* encrypt(char *text, char *key) {
    ...
    for (i = 0; i < strlen(text); ++i) {               //iterate over text and use
        temp[0] = get_encrypted_char(text[i], key[i]); //the key char to convert the
        strcat(encrypted_text, temp);                  //text char
    }
...
void verify_encrypt(int newsockfd) {
    ...
    if(verify_size != size)
        _exit(1);             //It's safe to exit, since if this is called, it's a child
